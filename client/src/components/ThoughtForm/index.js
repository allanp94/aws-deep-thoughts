...
const ThoughtForm = () => {
  ...
  const handleImageUpload = (event) => {
    ...
    data.append("image", fileInput.current.files[0]); // get the most current uploaded image
    ...
    const postImage = async () => {
      try {
        ...
        return postResponse.Location; //returns the location URL of the image in the s3 bucket
      } catch (error) {
        console.log(error);
      }
    };
